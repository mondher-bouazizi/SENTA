...
public class NaiveBayesUpdateableClassifier {
	...
	private static NaiveBayesUpdateable naiveBayes;
	...
	public static void setParameters() {
		...
		naiveBayes.setBatchSize(ClassifierParameters.getNbBatchSize() + "");
		naiveBayes.setDebug(ClassifierParameters.isNbDebug());
		naiveBayes.setDisplayModelInOldFormat(false);
		naiveBayes.setDoNotCheckCapabilities(ClassifierParameters.isNbDoNotCheckCapabilities()); // TODO check where this parameter has gone
		naiveBayes.setNumDecimalPlaces(ClassifierParameters.getNbNumDecimalPlaces());
		naiveBayes.setUseKernelEstimator(ClassifierParameters.isNbUserKernelEstimator());
		naiveBayes.setUseSupervisedDiscretization(ClassifierParameters.isNbUseSupervisedDisc());
	}
	...
	public static void crossValidate(String arffFile, ArrayList<String> classes) {
		...
		try {
			...
			xEvaluation.crossValidateModel(naiveBayes, trainingData, 4, trainingData.getRandomNumberGenerator(50));

			StringBuffer results = new StringBuffer();
			
			results.append("== Classification using NAIVE BAYES UPDATEABLE ==\n\n");
	...
	public static void trainingSplit(String arffTrainingFile, int percentage, ArrayList<String> classes) {
		...
		try {
			...
			testEvaluation.evaluateModel(naiveBayes, testData);
			...
			for (int i = 0; i < testData.numInstances(); i++) {
				// label instances
				Double genderLabel = naiveBayes.classifyInstance(testData.instance(i));
	...
	public static void setOutputText(StringProperty outputText) {
		NaiveBayesUpdateableClassifier.outputText = outputText;
	}
